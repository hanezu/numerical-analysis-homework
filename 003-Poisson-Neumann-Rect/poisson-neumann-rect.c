// neumann condition u_y = 0 on y = 0.
// dirichlet condition u = 0 on other boundaries
...
int main(int argc, char * argv[]) {
	// 右辺ベクトルFhの初期化: Fh一旦初始化就不再改变！只是迭代求解U而已
	for (int i = 1; i < N; ++i) {
		for (int j = 1; j < N; ++j) {
	...
	// ヤコビ法による反復計算で方程式 Ah * uh = Fh を解く p19的例2.1
    // 注意：Ah在这里没有被显式地写出来，虽然很讨厌但是计算效率应该高？
		...
		for (int i = 1; i < N; ++i) {
			for (int j = 1; j < N; ++j) {
				...
				if (j > 1) {
                    ...
                    // j == 1, here lambda_Y = 0, lambda_Z = 1
                    // phi_N = 0, k = 0, |A-X|=h を仮定する
                    // the original equation was reduced to
                    // -u(Y)/h + u(A)/h = 0
                    // which is u(Y) == u(H)
